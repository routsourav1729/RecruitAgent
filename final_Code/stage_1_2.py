...
Usage from master:
    from stages.gemini_parser import GeminiParser
    
    # For CVs
    cv_parser = GeminiParser(
        model=gemini_model,
        prompt_file="prompts/stage_1_1.txt"
    )
    cv_json = cv_parser.process_text(cv_text, "resume.pdf")
    
    # For JDs  
    jd_parser = GeminiParser(
        model=gemini_model,
        prompt_file="prompts/stage_1_2.txt"
    )
    jd_json = jd_parser.process_text(jd_text, "job_description.pdf")
"""
